...
EventParser._parseContentDirectoryEvent = function (body, device) {
  const eventData = body.reduce((event, entry) => Object.assign(event, entry), {})
  const event = { name: 'ContentDirectory', eventBody: eventData }
  device.emit(event.name, eventData)
  return event
}
