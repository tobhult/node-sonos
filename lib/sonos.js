 ...
 * @returns  {Object} {returned: {String}, total: {String}, items:[{title:{String}, uri: {String}}]}
 */
Sonos.prototype.getQueue = async function (queueId) {
  var options = {
    BrowseFlag: 'BrowseDirectChildren',
    Filter: '*',
    StartingIndex: '0',
 ...
 * @returns  {Object} {returned: {String}, total: {String}, items:[{title:{String}, uri: {String}}]}
 */
Sonos.prototype.getAllQueues = async function () {
  var options = {
    BrowseFlag: 'BrowseDirectChildren',
    Filter: '*',
    StartingIndex: '0',
  ...
  return this.contentDirectoryService().GetResult(options).then(result => {
    return Promise.all(result.items.map(
      item => this.getQueue(item.id)
        .then(queue => {
          queue.uri = item.uri
          return queue
        })))
